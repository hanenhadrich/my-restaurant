...
export const fetchProduits = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
      try {
        const response = await axios.get("http://localhost:3000/repasGourmet"); 
...
export const fetchProduitById = createAsyncThunk("produits/fetchProduitById", async (produitId, { rejectWithValue }) => {
    try {
        const response = await axios.get(`http://localhost:3000/repasGourmet/${produitId}`);
        return response.data;
    } catch (error) {
...
export const createProduit = createAsyncThunk("produits/createProduit", async (newProduit, { rejectWithValue }) => {
    try {
        const response = await axios.post("http://localhost:3000/repasGourmet", newProduit);
        return response.data;
    } catch (error) {
...
export const updateProduit = createAsyncThunk("produits/updateProduit", async ({ produitId, newData }, { rejectWithValue }) => {
    try {
        const response = await axios.patch(`http://localhost:3000/repasGourmet/${produitId}`, newData);
        return response.data;
    } catch (error) {
...
export const deleteProduit = createAsyncThunk("produits/deleteProduit", async (produitId, { rejectWithValue }) => {
    try {
        await axios.delete(`http://localhost:3000/repasGourmet/${produitId}`);
        return produitId;
    } catch (error) {
...
const repasGourmetSlice = createSlice({
    name: "produits",
    initialState,
    reducers: {
...
export const { unselectProduit } = repasGourmetSlice.actions;
export default repasGourmetSlice.reducer;
