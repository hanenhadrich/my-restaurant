...
import { fetchProduitById, unselectProduit, updateProduit } from "../store/produitsTraditionnelSlice"; // Corrected import
import { Spinner, Alert } from "react-bootstrap";
import ProduitForm from "../components/ProduitForm"; 

function UpdateProduitTraditionnel() {
  const { id } = useParams(); // Get the product ID from the URL
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const produit = useSelector((state) => state.produitsTraditionnel.selected); // Selected product from state
  const loading = useSelector((state) => state.produitsTraditionnel.loading); // Loading state
  const error = useSelector((state) => state.produitsTraditionnel.error); // Error state
...
export default UpdateProduitTraditionnel;
